...
class User(Base):
    ...
    created_at = Column(
        String, default=lambda: datetime.now(KST).strftime("%Y-%m-%d %H:%M:%S")
    )
    ...
    nick_name = Column(String, default="익명")
...
class Score(Base):
    ...
    played_date = Column(
        String, default=lambda: datetime.now(KST).strftime("%Y-%m-%d"), primary_key=True
    )
